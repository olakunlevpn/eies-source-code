...
use MasterStudy\Lms\Pro\AddonsPlus\AiLab\Http\Serializers\AiAssignmentSerializer;
use MasterStudy\Lms\Validation\Validator;

class GenerateAssignmentController extends Controller {
	public function __invoke( \WP_REST_Request $request ) {
		...
		try {
			$response = $this->ai->generate_assignment(
		...
		return new \WP_REST_Response(
			( new AiAssignmentSerializer() )->toArray( $response )
		);
	}
}
