<div class="stm_image_box stm_image_box__<?php echo esc_attr( $style . ' ' . $main_class ); ?>">
	...
	<?php if ( ! empty( $image ) ) : ?>
		<div class="stm_image_box__image">
			<?php echo wp_kses_post( html_entity_decode( stm_get_VC_attachment_img_safe( $image, $image_size ) ) ); ?>
			<?php if ( ! empty( $icon ) ) : ?>
	...
	<div class="stm_image_box__content">
		...
		if ( function_exists( 'vc_build_link' ) && ! defined( 'ELEMENTOR_PLUGIN_BASE' ) ) {
			...
			if ( ! empty( $button ) && ! empty( $button['title'] ) && ! empty( $button['url'] ) ) {
				?>
					<a href="<?php echo esc_url( $button['url'] ); ?>" class="btn btn-default" <?php echo esc_attr( 'on' === $button['is_external'] ? 'target="_blank"' : '' ); ?>>
